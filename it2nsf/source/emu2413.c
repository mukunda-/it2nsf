...
static void makePmTable(OPLL *opll) {
    ...
    for (i = 0; i < PM_PG_WIDTH; i++)
        opll->pmtable[i] = (e_int32) ((double) PM_AMP *
...
static void makeDphaseTable(OPLL *opll) {
    ...
    e_uint32 mltable[16] =
            {1, 1 * 2, 2 * 2, 3 * 2, 4 * 2, 5 * 2, 6 * 2, 7 * 2, 8 * 2, 9 * 2, 10 * 2, 10 * 2, 12 * 2, 12 * 2, 15 * 2,
             15 * 2};
...
#define dB2(x) ((x)*2)
    ...
    static const double kltable[16] = {
            dB2 (0.000), dB2 (9.000), dB2 (12.000), dB2 (13.875), dB2 (15.000), dB2 (16.125), dB2 (16.875),
            dB2 (17.625),
            dB2 (18.000), dB2 (18.750), dB2 (19.125), dB2 (19.500), dB2 (19.875), dB2 (20.250), dB2 (20.625),
            dB2 (21.000)
    };
...
INLINE static void slotOff(OPLL *opll, OPLL_SLOT *slot) {
    if (slot->eg_mode == ATTACK)
        slot->eg_phase = EXPAND_BITS (opll->AR_ADJUST_TABLE[HIGHBITS(slot->eg_phase, EG_DP_BITS - EG_BITS)], EG_BITS,
                                      EG_DP_BITS);
...
INLINE static e_int32 calc_slot_car(OPLL *opll, OPLL_SLOT *slot, e_int32 fm) {
    ...
    if (slot->egout >= (DB_MUTE - 1)) {
        ...
        slot->output[0] = opll->DB2LIN_TABLE[slot->sintbl[(slot->pgout + wave2_8pi(fm)) & (PG_WIDTH - 1)] +
                                             slot->egout];
    }
